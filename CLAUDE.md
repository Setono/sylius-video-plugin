...
- **Cloudflare Stream (opt-in type).** `Model\CloudflareStreamProductVideo` stores only a `uid`: the
  ...
  for a one-time tus upload URL; the same client manages the account's webhook behind
  `CloudflareStream\WebhookClientInterface`). Readiness comes from Symfony's Webhook component — `Webhook\CloudflareStreamRequestParser`
  (signature checked by `CloudflareStream\WebhookSignatureVerifierInterface` against the configured `webhook_secret` or,
  by default, the secret `CloudflareStream\WebhookSecretProviderInterface` reads back from Cloudflare and remembers in
  `cache.app`; routed by the `framework.webhook.routing` entry the extension prepends whenever the type is enabled) and
  `Webhook\CloudflareStreamWebhookConsumer` — or `Command\CloudflareStreamSyncCommand`.
  `Command\CloudflareStreamSubscribeWebhookCommand` points the account's single webhook at the application, idempotently;
...
- Twig folders and file names are snake_case (`admin/form/_videos_theme.html.twig`, `shop/renderer/url.html.twig`); never camelCase. `tests/Unit/Resources/TwigFileNamesTest` enforces it (Sylius template overrides under `tests/Application/templates/bundles` are exempt because they mirror Sylius paths).
